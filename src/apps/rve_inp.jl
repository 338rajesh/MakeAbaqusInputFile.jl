...
function write_unit_cell_inp(
    ...
    for a_req_prop in required_properties
        ...
        options = get_the_load_vector_3D(a_req_prop, options, (max_strain*lx, max_strain*ly, max_strain*lz))
        # ==============================
        #       Start of INP file
        # ==============================
        ...
        add_constraint_equations(
            ...
            mesh_data["npairs"], mesh_data["ntags"], mesh_data["ncoor"])
        ...
        add_boundary_conditions(inp_file, analysis_type, options)
        #
...
function get_the_load_vector_3D(
    ...
    displacements::NTuple{3,Float64},
)
    ...
    elseif prop == "E11_3D"
        options[:rel_disp_matrix][3, 3] = displacements[3]
    elseif prop == "G31_3D"
        options[:rel_disp_matrix][2, 3] = displacements[3]
        options[:rel_disp_matrix][3, 2] = displacements[2]
    elseif prop == "G12_3D"
        options[:rel_disp_matrix][1, 3] = displacements[3]
        options[:rel_disp_matrix][3, 1] = displacements[1]
    elseif prop == "G23_3D"
